                ...
                <CardContent sx={{ p: { xs: 3, sm: 4 }, flexGrow: 1 }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {[
                      ...
                      { icon: <FaBirthdayCake size={18} />, label: "Age", value: patient.dateOfBirth ? `${calculateAge(patient.dateOfBirth)} years` : "Not specified" },
                    ].map((item, idx) => (
                ...
                <CardContent sx={{ p: { xs: 3, sm: 4 }, flexGrow: 1 }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {[
                      ...
                      { icon: <FaCalendarAlt size={18} />, label: "Member Since", value: patient.registerDate ? dayjs(patient.registerDate).format("DD/MM/YYYY") : "Not specified" },
                    ].map((item, idx) => (
                      <Box key={idx} sx={{ display: "flex", alignItems: "center", gap: 2, p: 2, borderBottom: idx < 2 ? `1px solid rgba(184,151,42,0.08)` : "none" }}>
                        <Box sx={{ color: GOLD, display: "flex", alignItems: "center", justifyContent: "center" }}>{item.icon}</Box>
                        <Box sx={{ display: "flex", flex: 1, gap: 1, alignItems: "center" }}>
                          <Typography sx={{ color: TEXT_MID, fontWeight: 600, fontSize: "0.95rem" }}>{item.label}:</Typography>
                          <Typography sx={{ color: TEXT_DARK, fontWeight: 700, fontSize: "0.95rem", wordBreak: "break-all" }}>{item.value}</Typography>
