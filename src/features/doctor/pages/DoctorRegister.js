...
import {
    TextField, Button, Typography, Box, CircularProgress, Alert, Select, MenuItem,
...
import { doctorRegister, getUploadSignature } from '../../../services/authService';
...
const STEPS = [
    ...
    { label: "Profile", icon: <VerifiedUserIcon /> },
    { label: "Medical", icon: <WorkOutlineIcon /> },
];

/* ── Left-panel highlights ── */
const HIGHLIGHTS = [
    { icon: <SecurityIcon />, text: "Verified professional credentials" },
    { icon: <WorkOutlineIcon />, text: "Manage your clinic schedule effortlessly" },
    { icon: <LocalHospitalIcon />, text: "Connect with patients instantly" },
...
export default function DoctorRegister() {
    ...
    useEffect(() => {
        const fetchSpecialties = async () => {
            try {
                ...
                setSpecialties(responseData.map((item) => ({ id: item.id, name: item.name })));
            } catch (err) {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            await doctorRegister(registrationData);
            navigate("/");
        } catch (err) {
            ...
            <Box sx={{ overflowX: "hidden", display: "flex", minHeight: "100vh", fontFamily: "'Inter', sans-serif", bgcolor: "#f9f8f5" }}>
                ...
                <Box sx={{ display: { xs: "none", md: "flex" }, flexDirection: "column", justifyContent: "space-between", width: "40%", flexShrink: 0, position: "relative", overflow: "hidden", animation: "panelFadeIn 0.7s ease both" }}>
                    ...
                    <Box sx={{ position: "relative", zIndex: 1, p: { md: 5, lg: 7 } }}>
                        ...
                        <Typography sx={{ fontSize: { md: "1.9rem", lg: "2.4rem" }, fontWeight: 800, color: "white", lineHeight: 1.2, letterSpacing: "-0.5px", mb: 1 }}>Join Our Medical</Typography>
                        <Typography sx={{
                        ...
                        }}>Excellence Network.</Typography>
                        <Typography sx={{ color: "rgba(255,255,255,.6)", fontSize: "0.9rem", lineHeight: 1.7, maxWidth: 320, mb: 5 }}>
                            Enable a streamlined healthcare experience for your patients.
                            Manage records, schedules, and clinical visits effortlessly.
                        </Typography>
                        ...
                        <Box sx={{ mt: 6, p: 2.5, borderRadius: 2.5, bgcolor: `${GOLD}10`, border: `1px solid ${GOLD}25` }}>
                            <Typography sx={{ color: GOLD, fontSize: "0.7rem", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", mb: 1.5 }}>Registration Progress</Typography>
                    ...
                    <Box sx={{ width: "100%", maxWidth: 560 }}>
                        <Box sx={{ mb: 4 }}>
                            <Box sx={{ width: 56, height: 56, borderRadius: 3, background: `linear-gradient(135deg,${GOLD_LIGHT},#fdf3d0)`, border: `1.5px solid ${GOLD}40`, display: "flex", alignItems: "center", justifyContent: "center", mb: 2.5, animation: "floatY 4s ease-in-out infinite", boxShadow: `0 8px 24px ${GOLD}25` }}>
                                <WorkOutlineIcon sx={{ color: GOLD, fontSize: 26 }} />
                            </Box>
                            <Typography variant="h4" sx={{ fontWeight: 800, color: TEXT_DARK, letterSpacing: "-0.5px", mb: 0.5 }}>Doctor Portal</Typography>
                        ...
                        <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
                            ...
                            {step === 2 && (
                                <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5, animation: "stepSlide 0.35s ease both" }}>
                                    <FormControl fullWidth required error={!!errors.specialityId} className="gold-select" sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2.5, bgcolor: "#fafafa" } }}>
                                        <InputLabel>Specialty</InputLabel>
                                        <Select label="Specialty" defaultValue="" {...register("specialityId")} startAdornment={<InputAdornment position="start"><VerifiedUserIcon sx={{ color: GOLD, fontSize: 20, ml: 0.5 }} /></InputAdornment>}>
                                            {specialties.map((spec) => (<MenuItem key={spec.id} value={spec.id}>{spec.name}</MenuItem>))}
                                        </Select>
                                        {errors.specialityId && <Typography color="error" variant="caption" sx={{ mt: 0.5, ml: 1.5 }}>{errors.specialityId.message}</Typography>}
                                    </FormControl>
