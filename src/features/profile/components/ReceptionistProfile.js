...
export default function ReceptionistProfile({ user }) {
  const formattedShiftStart = user.shiftStart ? user.shiftStart : "Not set";
  const formattedShiftEnd = user.shiftEnd ? user.shiftEnd : "Not set";
      ...
      >
        ...
        <CardContent sx={{ p: { xs: 3, sm: 4 }, flexGrow: 1 }}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {[
              { icon: <FaClock size={18} />, label: "Start", value: formattedShiftStart },
              { icon: <FaClock size={18} />, label: "End", value: formattedShiftEnd },
            ].map((item, idx) => (
              <Box
                key={idx}
                sx={{
